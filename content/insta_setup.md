> **PARKED (2026-09):** Meta for Developers در دسترس صاحب پیج نیست (محدودیت جغرافیایی).
> مسیر رسمی فعلی انتشار، **Buffer** است: `build/buffer_publish.py` +
> workflowهای `daily-trend-draft` و `publish-approved-draft` (تأیید با `/publish` در Issue روزانه).
> این فایل و `build/insta_publish.py` فقط برای مرجع نگه داشته شده‌اند و هیچ‌جا اجرا نمی‌شوند.
